...
Feature: ProcessSalariesDepositSlip
  ...
  @ProcessSalariesDepositSlip @ProcessSalariesDepositSlip4
  ...
  @debug
  Scenario: Employer downloads the other bank salary file using the establishment ID, modifies the file and generates the SIF file, then submits it for approval. The admin checks the salary approval and verifies both the original file and the SIF file at the checker and authorizer levels.
    Given   [Login Page] User enter the email ""
    When    [Login Page] User enter the password ""
    Then    [Login page] User tap on login button
    And     [Sub Admin] User tap on the submit button
