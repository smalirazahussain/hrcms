...
Feature: AddEmployer
  ...
  Scenario: Add new employer with Centiv Card
   ...
   # Then    [Add Employer] user select the nationality "Afghanistan"
    ...
    Then    [Request Page] User tap on the view button
    And     [Request Page] User verify the mol and passport no and then click the approve button
    Then    [Admin Page] Open the admin tab "http://192.168.105.105:3001/"
    And     [Admin Page] User tap on login page
    Then    [Admin Page] User enter the  email "admin@admin.com"
    When    [Admin Page] User enter the Password "12345678"
    And     [Admin Page] User tap on login page
  ...
  Scenario: Add new employer with other bank validation ""Arab African International Bank""
    Given   [Login Page] User enter the email "12992"
    When    [Login Page] User enter the password "Password1"
    Then    [Login page] User tap on login button
    Then    [Employees Page] User tap on employees button
    Given   [Add Employer] user tap on new employer button
 ...
 Scenario Outline: User create a excel and enter the multiple employee data and check the dulpication data then approve the admin side
   ...
   Then    [Admin Page] User select the card scheme "PCMS"
...
#    And     [Request Page] User tap on the Approve button
    ...
    Examples:
      |FirstName|MolNo   |EmpCode|LastName|DisplayName|DOB       |Gender|Nationality|JoiningDate|Email          |MobileNo|AlternatePhone|HomeAddress|HomeState|HomePostCode|WorkAddress|WorkState|WorkPostCode|PassportNo|PassportExpiry|EID|EIDExpiry |EstablishmentId |
      |User     |Mol12345|1      |1       |User       |05/05/1978|M     |BD         |05/05/1978 |@mailinator.com|971     |971           |Address    |1        |00001       |2          |2        |2           |Passport  |10/05/2029    |   |10/05/2029|HyperDynamics8520268   |
  ...
  Scenario Outline: Create multiple Employee with non wps process
    ...
    And     [Admin Page] User tap on login page
    Then    [Admin Page] User enter the  email "admin@admin.com"
    When    [Admin Page] User enter the Password "P@ssword12345"
    And     [Admin Page] User tap on login page
    Then    [Sign Up] User enter the otp code "111111"
    When    [Admin Page] User tap on client Approvals
    Then    [Admin Page] User enter the company name
    ...
    Examples:
      |FirstName|MolNo|EmpCode|LastName|DisplayName|DOB|Gender|Nationality|JoiningDate|Email|MobileNo|AlternatePhone|HomeAddress|HomeState|HomePostCode|WorkAddress|WorkState|WorkPostCode|PassportNo|PassportExpiry|EID|EIDExpiry|EstablishmentId|EmployeeCount|
      |User|00000000000000|0000|1||01/11/1978|M|BD|05/05/1978|@mailinator.com|97156|97156|Address|1|00001|4|2|3|Passport|10/05/2029|78419910|10/05/2029|123456789123901|10|
  ...
  Scenario Outline: Create multiple Employee for the other bank
    ...
    Examples:
      ...
      | 0000             |         |           |          |             |            | M      |             | 10/01/2020  | john@mailinator.com | 97150001 | 97150091       | HomeSt 123  | Dubai     | 00001        | WorkPl 45   | Sharjah   | 00002        | P1234567   | 10/05/2029     | 7841991001 | 10/05/2029  | HyperDynamics8520268       |
